        ...
        for idx in range(start_idx, len(results)):
            ...
            while True:
                ...
                try:
                    ...
                    sleep_seconds(cfg.delay_seconds)
                    indicators = otx.get_pulse_indicators(pulse_id, limit=1000, include_inactive=True)

                    record = normalize_pulse_full(pulse)
                    iocs = normalize_indicators(indicators)
                    ...
                    if not iocs:
                        logger.info("Pulse %s has no IPv4/IPv6 IOCs; skipping storage", pulse_id)
                        store.mark_seen(pulse_id)
                        _write_checkpoint(cfg.checkpoint_path, ckpt)
                        break
