...
IP_INDICATOR_TYPES: frozenset[str] = frozenset({"IPv4", "IPv6"})


def _is_ip_indicator_type(indicator_type: str) -> bool:
    return indicator_type in IP_INDICATOR_TYPES
...
def normalize_indicators(indicators: list[dict[str, Any]]) -> list[PulseIocRecord]:
    ...
    for item in indicators:
        if not isinstance(item, dict):
            continue
        indicator_type = _first_str(item.get("type"))
        if not _is_ip_indicator_type(indicator_type):
            continue
        out.append(
            {
